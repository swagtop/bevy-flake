{
  ...
  let
    devShellFor = system: let
      ...
      # Returns pkg-config search path for Linux headers of specific arch.
      linuxHeaders = system:
      ...
      cargo-wrapper = pkgs.writeShellScriptBin "cargo" ''
        ...
        case $BEVY_FLAKE_TARGET in
          ...
          aarch64-unknown-linux-gnu*)
            PKG_CONFIG_PATH="${linuxHeaders "aarch64-linux"}:$PKG_CONFIG_PATH"
            RUSTFLAGS="${crossFlags} $RUSTFLAGS"
          ;;
          x86_64-unknown-linux-gnu*)
            PKG_CONFIG_PATH="${linuxHeaders "x86_64-linux"}:$PKG_CONFIG_PATH"
            RUSTFLAGS="${crossFlags} $RUSTFLAGS"
          ;;
          wasm32-unknown-unknown)
  ...
  in {
    devShells = {
      ...
      "aarch64-linux".default = devShellFor "aarch64-linux";
    };
  };
}
